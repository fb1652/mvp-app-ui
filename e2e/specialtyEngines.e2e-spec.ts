...
describe('MRI', () => {
  ...
  // const specialtyEnginesPage = new SpecialtyEnginesPage();
  const stepsPage = new StepsPage();
  const datacenterName = `E2ENAME_ZIIPS_${new Date().getTime()}`;

  describe('specialty engines', () => {

    beforeAll(() => {
      return appPage.login()
      .then(() => catalogPage.toSpecialtyEngines());
    });

    it('should have the correct overview title', () => {
      return overviewPage.getPageTitle('Specialty Engines')
      .then((title: string) => {
        expect(title).toEqual('Specialty Engines MRI');
      });
    });
